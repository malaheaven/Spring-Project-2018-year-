...
<body id="page2">
	<div class="extra">
		<div class="main">
			...
			<nav>
				<ul id="menu">
					...
					<li><a href="../member/updatemember.jsp" class="nav2">회원정보
							수정</a></li>
					<li><a href="../member/pwCheck.jsp" class="nav3">회원탈퇴</a></li>
		...
		return a;
			...
			<%
				ArrayList<ReviewDTO> list = (ArrayList<ReviewDTO>) session.getAttribute("myReview");
			%>
			<%
			...
			<div id="mainWrapper">
				...
				<ul>
					<!-- 게시판 제목 -->
					<li>내가 작성한 리뷰</li>
					<button onclick="location.href = '../review/review.jsp'" class="button"
					...
					<li>
						<ul id="ulTable">
							...
							<!-- 게시물이 출력될 영역 -->
         <%
                        int endIndex = record_end_no;
                        if (record_end_no > list.size()-1) {
                           endIndex = record_start_no + (list.size() - record_start_no);
					...
					<li>
						<div id="divPaging">
							...
							<a href="myReviewResult.jsp?pageno=<%=prev_pageno%>">◀</a>

							<%
								for (int i = page_sno; i <= page_eno; i++) {
							%>
							<a href="myReviewResult.jsp?pageno=<%=i%>"> <%
 	if (pageno == i) {
 %> <b>[<%=i%>]
							</b> <%
 	} else {
 ...
 %>
							...
							<a href="myReviewResult.jsp?pageno=<%=next_pageno%>">▶</a>
						</div>
					</li>
				</ul>
			</div>
